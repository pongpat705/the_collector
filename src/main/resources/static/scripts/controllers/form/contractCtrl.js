...
angular
	.module('app')
		.controller('contractCtrl', [	'$scope', '$http', '$localStorage', 
									'$timeout', '$translate', 
									'$state' , '$stateParams', 'Restangular', 
									'toastr', '$rootScope', 'formService',
  function contractCtrl($scope, $http, $localStorage, 
		  			$timeout, $translate, 
		  			$state, $stateParams, Restangular, 
		  			toastr, $rootScope, formService) {
	
	$scope.$watch("init", function(){
		$scope.formName = $rootScope.moneyControlForms.CONTRACT_COLLATERAL.value1;
		$scope.getAllNbutGet();
	});
